...
def run_(data, dataset,edge_index, train_node,writer, runs=10, lr=0.01, weight_decay=0.005):
    ...
    for i in range(500):
        ...
        train(model, optimizer, data, edge_index,writer,i)
    if torch.cuda.is_available():
        torch.cuda.synchronize()
...
def train(model, optimizer, data, edge_index,writer,epoch):
    model.train()
    optimizer.zero_grad()
    ...
    test_loss = F.nll_loss(out[y_mask], data.y[y_mask]).item()
