...
def train(model, optimizer, data):
    ...
    out, score_loss= model(data, pos_edge_index, neg_edge_index)
...
def evaluate(model, data):
    ...
    for prefix in ["test"]:
        ...
        out,_ = model(data, pos_edge_index, neg_edge_index.to(device))
        _, indices = torch.topk(out, 500)
        # pdb.set_trace()
    ...
    return roc_auc_score(link_labels, link_probs), precision_recall_fscore_support(link_labels, link_probs>0.5), loss, confusion_matrix(link_labels, link_probs>0.8), edge_index_cs
