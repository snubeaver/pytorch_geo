...
def evaluate(model, data):
    ...
    with torch.no_grad():
        ...
        logits, z, n =model(data, pos_edge_index, neg_edge_index.to(device), data.train_edge_index)
    y_mask = index_to_mask(tensor(data.test_mask, device=logits.device), size=data.num_nodes)
...
class MyDataLoader(torch.utils.data.DataLoader):
    ...
    def __init__(self, dataset, batch_size=1, shuffle=False, collate_fn=MyCollater(), num_workers=0, **kwargs):
